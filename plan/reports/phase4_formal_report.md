...
## 8. 事后审计：方法局限与风险（2026-07-02 更新）

> 以下内容基于交叉验证（42/43）和 RTL bug 修复的实践经验补充。原始报告（§7）的结论在形式化方法的范围内是正确的，但方法的局限性当时未被充分披露。

### 8.1 形式化方法的两个盲区

| 盲区 | 原始配置 | 风险 |
|------|---------|------|
| **证明深度不足** | `-seq 4`（4 周期归纳） | FSM 执行一条指令需 5-7 周期，4 周期窗口无法覆盖完整指令，跳转/循环/栈操作等跨指令 bug 无法被归纳捕获 |
| **标准单元 Blackbox** | `sky130_fd_sc_hd__blackbox.v` 模型 | 网表 8,544 个单元全部使用 blackbox 模型，Yosys SAT 对其内部逻辑不做推理。`equiv_induct` 仅在 blackbox 边界假设等价，不验证单元内部实现 |
| **组合效应** | `-seq 4` + `blackbox` | 两个盲区叠加：4 周期只覆盖 RTL 侧的部分状态空间，而 gate 侧由 blackbox 模型"保证"等价——实际上 gate 侧的逻辑根本没有被检验 |

### 8.2 实际发现的差距

尽管形式化报告结论为"100% 内部逻辑等价"，后续交叉验证（43 条指令仿真测试）发现了 **15 个 RTL 功能 bug**：

| 类别 | 数量 | 典型 bug |
|------|:---:|------|
| 指令译码 | 6 | 7-bit 截断导致跨指令误匹配 |
| 状态机 | 1 | FETCH2 条件反转 |
| 数据通路 | 2 | internal_bus 多驱冲突、WRITEBK 双写 |
| 寄存器文件 | 1 | R0-R7 读写路径缺失 |
| ALU | 2 | opcode 截断、DJNZ ALU op 错误 |
| 栈操作 | 2 | SP 每周期累加、PUSH 数据路径 |
| 时序 | 1 | exec_en/fetch_en 组合逻辑竞争 |

交叉验证从 5/43 修复到 42/43（剩余 1 个为 trace 时序偏移，非功能 bug）。

### 8.3 为什么形式化没发现这些 bug

1. **`-seq 4` 太短**：PC 计数错位需要 6+ 周期（跨 2 条指令）才暴露；SP 累加需要 12+ 周期（跨 PUSH 的 6 个状态）；DJNZ 循环需要 30+ 周期
2. **`blackbox` 消去了 gate 侧证据**：当 gate 侧是 unknown cell 时，`equiv_induct` 假设等价成立，不深入检查。全芯片 8,544 个 cell 全是 blackbox，实际验证深度为零
3. **等价性成立的充要条件被打破**：`equiv_induct` 的归纳假设要求两个设计的状态空间在 4 周期内对齐。当 RTL 的 PC 逻辑和 gate 的 PC 逻辑在 4 周期内恰好输出相同值（都在取指阶段），差异只在第 5 周期（执行阶段）才暴露

### 8.4 方法论改进建议

| 问题 | 改进 |
|------|------|
| `-seq` 深度 | 至少设为 `-seq 20`（覆盖 4 条完整指令），或使用 `equiv_induct -seq 50` 覆盖整个交叉验证程序 |
| blackbox 模型 | 使用 `read_liberty` + `dfflibmap` + `abc` 将 gate 映射回通用 cell，使 SAT solver 能够推理 gate 内部逻辑 |
